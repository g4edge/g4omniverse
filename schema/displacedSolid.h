...
//
#ifndef G4_GENERATED_DISPLACEDSOLID_H
#define G4_GENERATED_DISPLACEDSOLID_H

/// \file G4/displacedSolid.h
...
///
///
class G4DisplacedSolid : public UsdGeomXform
{
public:
    ...
    /// Construct a G4DisplacedSolid on UsdPrim \p prim .
    /// Equivalent to G4DisplacedSolid::Get(prim.GetStage(), prim.GetPath())
    /// for a \em valid \p prim, but will not immediately throw an error for
    /// an invalid \p prim
    explicit G4DisplacedSolid(const UsdPrim& prim=UsdPrim())
        : UsdGeomXform(prim)
    {
    }

    /// Construct a G4DisplacedSolid on the prim held by \p schemaObj .
    /// Should be preferred over G4DisplacedSolid(schemaObj.GetPrim()),
    /// as it preserves SchemaBase state.
    explicit G4DisplacedSolid(const UsdSchemaBase& schemaObj)
    ...
    /// Return a G4DisplacedSolid holding the prim adhering to this
    /// schema at \p path on \p stage.  If no prim exists at \p path on
    /// \p stage, or if the prim at that path does not adhere to this schema,
    /// return an invalid schema object.  This is shorthand for the following:
    ///
    /// \code
    /// G4DisplacedSolid(stage->GetPrimAtPath(path));
    /// \endcode
    ///
    G4_API
    static G4DisplacedSolid
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `string g4type = "DisplacedSolid"` |
    /// | C++ Type | std::string |
